...
#ifndef PDBNTK_FACTOR_GRAPH_H_ 
#define PDBNTK_FACTOR_GRAPH_H_
...
public:
  ...
  const dai::Neighbors& nbV( size_t i ) const { return _G.nb1(i); }
  /// Returns constant reference to neighbors of the \a I 'th factor
  const dai::Neighbors& nbF( size_t I ) const { return _G.nb2(I); }
  /// Returns constant reference to the \a _I 'th neighbor of the \a i 'th variable
  const dai::Neighbor& nbV( size_t i, size_t _I ) const { return _G.nb1(i)[_I]; }
  /// Returns constant reference to the \a _i 'th neighbor of the \a I 'th factor
  const dai::Neighbor& nbF( size_t I, size_t _i ) const { return _G.nb2(I)[_i]; }
  //@}
  ...
  */
  ...
  NodeSet Delta( const NodeSet& vs ) const;
  ...
  */
  ...
  /** \throw MULTIPLE_UNDO if a backup already exists
  */
  void backupFactors( const NodeSet& ns );

  /// Restores all factors connected to a set of variables from their backups
  void restoreFactors( const NodeSet& ns );
  //@}
