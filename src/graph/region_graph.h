...
class Region : public NodeSet {
  ...
  public:
    /// Default constructor
    Region() : NodeSet(), _c(1.0) {}

    /// Construct from a set of variables and a counting number
    Region( const NodeSet& x, Real c ) : NodeSet(x), _c(c) {}
...
class RegionGraph : public FactorGraph {
  ...
  public:
    ...
    */
    RegionGraph( const FactorGraph& fg, const std::vector<NodeSet>& ors, const std::vector<Region>& irs, const std::vector<std::pair<size_t,size_t> >& edges ) : FactorGraph(), _G(), _ORs(), _IRs(), _fac2OR() {
     ...
     */
    RegionGraph( const FactorGraph& fg, const std::vector<NodeSet>& cl ) : FactorGraph(), _G(), _ORs(), _IRs(), _fac2OR() {
      constructCVM( fg, cl );
    ...
    const dai::Neighbors& nbOR( size_t alpha ) const { return _G.nb1(alpha); }

    /// Returns constant reference to the neighbors of inner region \a beta
    const dai::Neighbors& nbIR( size_t beta ) const { return _G.nb2(beta); }
     ...
     */
    const dai::BipartiteGraph& DAG() const { return _G; }
    //@}
