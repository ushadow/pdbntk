...
void RegionGraph::constructCVM( const FactorGraph &fg, const std::vector<NodeSet> &cl, size_t verbose ) {
    if( verbose )
    ...
    set<NodeSet> new_betas;
    do {
        new_betas.clear();
        for( set<NodeSet>::const_iterator gamma = betas.begin(); gamma != betas.end(); gamma++ )
            for( set<NodeSet>::const_iterator gamma2 = gamma; (++gamma2) != betas.end(); ) {
    ...
    for( set<NodeSet>::const_iterator beta = betas.begin(); beta != betas.end(); beta++ )
