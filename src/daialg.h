...
#ifndef PDBNTK_INF_ALG_H_ 
#define PDBNTK_INF_ALG_H_ 
...
class InfAlg {
    ...
    //@{
         ...
         */
        virtual InfAlg* construct( const FactorGraph &fg, const dai::PropertySet &opts ) const = 0;
    //@}
    ...
    //@{
         ...
         */
        virtual void init( const NodeSet &vs ) = 0;
         ...
         */
        virtual Factor belief( const NodeSet &vs ) const = 0;
         ...
         *  \note Before this method is called, run() should have been called.
         */
        virtual Factor beliefF( size_t I ) const { return belief( fg().factor(I).nodes() ); }
    ...
    //@{
        ...
        /** \throw MULTIPLE_UNDO if a backup already exists
         */
        virtual void backupFactors( const NodeSet &vs ) = 0;
        ...
        virtual void restoreFactors( const NodeSet &vs ) = 0;
    //@}
    ...
    //@{
         ...
         */
        virtual void setProperties( const dai::PropertySet &opts ) = 0;
        /// Returns parameters of this inference algorithm converted into a PropertySet.
        virtual dai::PropertySet getProperties() const = 0;
...
class DAIAlg : public InfAlg, public GRM {
    ...
    //@{
        ...
        /// Make backup copies of all factors involving the variables in \a vs
        void backupFactors( const NodeSet &vs ) { GRM::backupFactors( vs ); }

        /// Restore factor \a I from its backup copy
        void restoreFactor( size_t I ) { GRM::restoreFactor( I ); }
        /// Restore the factors involving the variables in \a vs from their backup copies
        void restoreFactors( const NodeSet &vs ) { GRM::restoreFactors( vs ); }
    //@}
};
 ...
 */
Factor calcMarginal( const InfAlg& obj, const NodeSet& vs, bool reInit );
 ...
 */
std::vector<Factor> calcPairBeliefs( const InfAlg& obj, const NodeSet& vs, bool reInit, bool accurate=false );
